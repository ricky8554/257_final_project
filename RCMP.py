...
class RCMP:
    def __init__(self,env,nhead = 5,epoch = 1000,vt = 0.11,missing_rate = 0, limit = 800):
        ...
        self.reference = PI(env).policy_iteration()
        self.var_history = []
    ...
    def train(self):
        ...
        for i in range(self.epoch):
            ...
            if i % 20 == 0:
                self.var_history.append( (i,np.mean(self.tmp_var_histroy)) )
                self.histroy.append( (i,self.eval()) )
                self.tmp_var_histroy.clear()
    ...
    def var(self,s):
        with torch.no_grad():
            vals = [self.agents[h](s)for h in range(self.nhead)]
            v = 0
         
            for i in range(4):
                v += np.var([float(vals[h][i].data) for h in range(self.nhead)])
            return v / 4
                

    def choose_action(self, s, head = None, eval = False):
        var = self.var(s)
        self.tmp_var_histroy.append(var)

        if not eval and  var > self.vt and (np.random.rand(1) > self.missing) and self.advice_count < self.limit:
            self.advice_count += 1
            self.nadvice += 1
